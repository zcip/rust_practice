...
pub enum Queries {
    ...
    GetTopTenHottestMonth,
    GetTopTenColdestMonth,
}
...
pub async fn execute(pool: &Pool, query: Queries) -> Result<Vec<WeatherAgg>, Error> {
    ...
    web::block(move || {
        ...
        match query {
            ...
            Queries::GetTopTenHottestMonth => get_hottest_months(conn),
            Queries::GetTopTenColdestMonth => get_coldest_months(conn),
        }
    })
    .await?
    .map_err(error::ErrorInternalServerError)
